...
void Prim(Graph* G,Vertex* StartVertext,Graph* MST){
    ...
    PQNode StartNode = {0,StartVertext};
    ...
    while (!PQ_IsEmpty(PQ))
    {
        ...
        while (CurrentEdge!=NULL)
        {
            ...
            {
                PQNode NewNode = {CurrentEdge,TargetVertex};
...
void Kruskal(Graph* G,Graph* MST){
    ...
    while (!PQ_IsEmpty(PQ))
    {
        ...
        if(DS_FindSet(VertexSet[FromIndex]) != DS_FindSet(
            VertexSet[ToIndex])){
                AddEdge(MSTVertices[FromIndex],CreateEdge(MSTVertices[FromIndex],MSTVertices[ToIndex],CurrentEdge->Weight));
                AddEdge(MSTVertices[FromIndex],CreateEdge(MSTVertices[ToIndex],MSTVertices[FromIndex],CurrentEdge->Weight));
